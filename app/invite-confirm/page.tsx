...
// Monochrome animated neural background
...
export default function InviteConfirmPage() {
  ...
  useEffect(() => {
    // Check if there is an active session (verified invite links automatically log the user in)
    const verifySession = async () => {
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {
          setError('No active invitation session found or invitation link expired. Please request a new invitation link.');
        }
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Synchronize profile name, country, and mobile to public.profiles via endpoint
      ...
      setSuccess('Account fully set up! Welcome to Mindly AI. Redirecting to home...');
      setTimeout(() => {
        router.push('/');
      }, 1500);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-primary)] text-[var(--text-primary)]">
        <div className="text-center">
          ...
          <p className="text-sm text-[var(--text-secondary)]">Checking invitation details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex text-[var(--text-primary)] bg-[var(--bg-primary)]">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 bg-[var(--bg-primary)] relative py-12">
        <div className="w-full max-w-sm relative z-10 animate-fade-in-up">
          ...
          {(!error || success || (!error.includes('session') && !error.includes('expired'))) && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label htmlFor="confirm-mobile" className="block text-xs font-semibold text-[var(--text-muted)] mb-1.5 uppercase tracking-wider">Mobile Number (Optional)</label>
                <input
                  id="confirm-mobile"
                  type="tel"
                  value={mobile}
